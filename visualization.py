...
def get_suggested_visualization_response_from_ai(sql_query, user_question):

  model_name = 'gpt-3.5-turbo'
  chat_api = ChatOpenAI(temperature=0, streaming=True, model_name=model_name)
...
def plot_visualization_for_user_question(visualization_response,
                                         query_output_df):
...
def _plot_bar_chart(visualization_response, query_output_df):
  horizontal_chart = alt.Chart(query_output_df).mark_bar().encode(
      ...
      color=alt.Color(
          visualization_response['x_axis_column_name'],
...
def _plot_line_chart(visualization_response, query_output_df):
  query_output_df.set_index(visualization_response['x_axis_column_name'],
                            inplace=True)
